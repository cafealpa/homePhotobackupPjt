...
$global:homePhotoOldPython = $false
function Invoke-TestPython {
    if ($args[0] -eq '-c' -and $args[1] -like '*print(str(sys.version_info.major)*') { Write-Output '3.14' }
    ...
    if ($args[0] -eq '-c' -and $args[1].Contains('sys.argv[1]') -and $global:homePhotoOldPython) { $global:LASTEXITCODE = 1 }
...
try {
    foreach ($file in @('install-search.ps1','start-installed-search.ps1','ml-worker/start-search.ps1')) {
    ...
    if ($PythonExe) {
        $probe = [scriptblock]::Create($source.Substring(0, $source.IndexOf('$worker =')))
        & $probe -InstallDir $fixture -PythonExe $PythonExe
    }
    ...
    & $installer -InstallDir $fixture -PythonExe 'mock'
    $global:homePhotoOldPython = $true
    & $installer -InstallDir $fixture -PythonExe 'mock'
    $global:homePhotoOldPython = $false
    if (@(Get-ChildItem -LiteralPath (Join-Path $fixture 'ml-worker') -Directory -Filter '.venv-search-backup-*').Count -ne 1) { throw 'Old venv was not backed up' }
    ...
    Write-Host 'PASS: syntax, pinned downloads, repeated install, Python migration backup, config preservation, DPAPI reuse, model failure.'
} finally {
    ...
    Remove-Variable homePhotoOldPython -Scope Global
}
