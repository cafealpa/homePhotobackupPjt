...
rem start-server.bat 이 같이 띄운 homephoto-ml-worker.exe (또는 개발용 python worker.py) 를 찾아 종료한다.
rem (콘솔 창 제목은 Windows Terminal 환경에서 믿을 수 없어 명령줄로 찾는다)
powershell -NoProfile -Command "$p = Get-CimInstance Win32_Process | Where-Object { $_.Name -like 'homephoto-ml-worker*' -or ($_.Name -like 'python*' -and $_.CommandLine -like '*worker.py*') }; if ($p) { $p | ForEach-Object { Stop-Process -Id $_.ProcessId -Force }; exit 0 } else { exit 1 }" >nul 2>&1
if errorlevel 1 (
