...
class TrashController(
    ...
    fun restore(@PathVariable id: Long): AssetDto {
        val restored = trashService.restore(id)
    ...
    fun empty(): Map<String, Any> {
        ...
        val purged = rows.count { trashService.purge(it[Assets.id]) }
        log.info("휴지통 비우기: {}건 영구 삭제", purged)
        return mapOf("purged" to purged)
    }
}
