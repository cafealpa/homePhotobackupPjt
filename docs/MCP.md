...
2026-09-14: 공개 OAuth 모드의 개발 후보 `0.1.6-rc1`을 추가했다. [OAuth 구현·검증·배포 안내](MCP-OAUTH.md)를 우선 참고한다. 아래 내용은 기존 로컬 모드 설명이며, 실제 ChatGPT 연결과 운영 배포는 아직 완료하지 않았다.
...
1. **외부 연결 방식**: PC가 켜져 있을 때만 사용하고, 무료 Duck DNS + 공유기 포트포워딩 + Caddy HTTPS로 연결하기로 결정했다. 클라우드에 사진/썸네일을 복제하지 않는다. `cho6253.duckdns.org`의 DNS 응답을 확인했으며 실제 집 PC의 HTTPS 연결은 대기 중이다. [설정 준비 문서](MCP-HOSTING.md)에 절차를 정리했다.
