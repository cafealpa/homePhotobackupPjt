...
- **설정 페이지 카드 레이아웃 (2026-08-19)**: 카테고리(서버 정보·저장소·보안·처리·장면 분석·임포트)를
  각각 접이식 카드로 만들고 `#set-cards`에 `columns: 400px 2`로 배치 — 카드 높이가 제각각이라 CSS 그리드는
  행마다 빈칸이 생긴다(멀티컬럼은 열을 채운다). 좁은 화면에서는 자동으로 1열. 저장 바는 멀티컬럼 밖에
  두어 항상 맨 아래 전체 폭을 쓰며, 저장 버튼은 `form="settings-form"`으로 폼과 연결한다.
  접힘 상태는 localStorage(`settingsCollapsedCards`)에 카드 id로 저장한다.
